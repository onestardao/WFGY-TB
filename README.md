...
With WFGY (two equivalent ways):

A) In-process Router (recommended, used in our runs)
   TB → WFGY MinAgent (delegates to Terminus) → LiteLLM(8080) → OpenAI
   (enable via: --agent-import-path wfgy.router_min:MinAgent)

B) Shell Wrapper (optional)
   TB → WFGY Router (wfgy_router.sh) → LiteLLM(8080) → OpenAI
...
**Option A — In-process Router (recommended, what we use for MVP and A/B):**

```bash
# Enable WFGY router & module
export WFGY_ENABLE_ROUTER=1
export WFGY_ROUTER_MODULE=wfgy.router_min
export WFGY_UPSTREAM_BASE_URL="http://127.0.0.1:8080/v1"
export WFGY_FIREWALL_MODE=auto   # auto | observe | strict

uvx --from terminal-bench tb run \
  --dataset-path "$DATASET_DIR" \
  --agent-import-path wfgy.router_min:MinAgent \
  --model openai/gpt-5 \
  --task-id hello-world \
  --n-attempts 1 \
  --n-concurrent 1 \
...
**Option B — Shell Wrapper (kept for compatibility):**
...
## 3b) Quick A/B switch (observe/strict) and not-fake verification

Use a Bash **array** for common args to avoid Click/CLI parsing issues like `Got unexpected extra arguments (1 1)`.
...
TASK=organization-json-generator

# A: delegate Terminus (WFGY inside MinAgent)
export WFGY_FIREWALL_MODE=auto
uvx --from terminal-bench tb run "${TB_ARGS[@]}" \
  --agent-import-path wfgy.router_min:MinAgent -t "$TASK" \
2>&1 | tee "runs/wfgy_A_${TASK}.log"

# B1: observe
export WFGY_FIREWALL_MODE=observe
uvx --from terminal-bench tb run "${TB_ARGS[@]}" \
  --agent-import-path wfgy.router_min:MinAgent -t "$TASK" \
2>&1 | tee "runs/wfgy_B_obs_${TASK}.log"

# B2: strict
export WFGY_FIREWALL_MODE=strict
uvx --from terminal-bench tb run "${TB_ARGS[@]}" \
  --agent-import-path wfgy.router_min:MinAgent -t "$TASK" \
2>&1 | tee "runs/wfgy_B_strict_${TASK}.log"

# Not-fake check: print panes head if present
RES=$(find runs -maxdepth 2 -name results.json | sort | tail -n1)
LOGDIR=$(jq -r '.trials[0].logging_dir // "null"' < "$RES")
if [ "$LOGDIR" != "null" ] && [ -f "$LOGDIR/panes/agent_out.txt" ]; then
  echo "--- panes head ---"; sed -n '1,3p' "$LOGDIR/panes/agent_out.txt"
else
  echo "no panes"
fi
```
...
* WFGY smoke prints stage markers and writes to `wfgy_logs` (shell wrapper) or `panes/agent_out.txt` contains `WFGY: MinAgent loaded`
...
**Got `unexpected extra arguments (1 1)`**
Use Bash arrays for common args (see **3b**) and keep boolean flags like `--rebuild` **without** `1`.
